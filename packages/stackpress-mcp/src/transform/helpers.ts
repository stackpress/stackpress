...
//same as stackpress-schema/transform/column/shape.ts ...
export const numbers = [ 'Number', 'Integer', 'Float' ];
...
export function getInputShape(column: Column, optional = false): string {
  optional = optional || !column.component.formField
  ...
  let shape = column.type.name in typemap
    ...
    ? `z.object({ ${
      column.type.fieldset.columns.map(
        column => getInputShape(column)
      ).toArray().join(', ')
    } })`
    : typemap.Unknown;
  //add description
...
export function getSelectors(
  type: 'filter' | 'span' | 'sort' | 'strings' | 'all', 
  model: Model, 
  prefix: string[] = []
) {
  ...
  if (type === 'filter') {
    ...
    model.store.spannables.forEach(column => {
      selectors.set([...prefix, column.name.toString()].join('.'), column);
    });
  } else if (type === 'sort') {
    ...
    model.columns.filter(column => Boolean(column.type.model)).forEach(column => {
      selectors.set([...prefix, column.name.toString()].join('.'), column);
    });  
  }
  model.store.foreignRelationships.forEach(column => {
    getSelectors(
      type, 
      column.store.foreignRelationship!.foreign.model, 
      [ ...prefix, column.name.toString() ]
    ).forEach((value, key) => {
...
export function getQuerySelectors(model: Model) {
  const filters = Array.from(getSelectors('filter', model).entries());
  const spans = Array.from(getSelectors('span', model).entries());
  const strings = Array.from(getSelectors('strings', model).entries());
  const sortables = Array.from(getSelectors('sort', model).entries());
  const columns = new Set<string>();
  Array.from(getSelectors('all', model).keys()).forEach(name => {
    const path = name.split('.');
    const star = path.length > 1 ? path.slice(0, -1).join('.') + '.*' : '*';
    columns.add(star);
    columns.add(name);
  });
  return { 
