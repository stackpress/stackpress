  ...
  implements ProfileStoreInterface
{
  ...
  public create() {
    ...
    schema.addField('name', {
      ...
      length: 255,
      nullable: false,
      comment: 'Full name (first middle last).'
    });
    schema.addField('type', {
      ...
      default: 'person',
      nullable: false,
      comment: 'Type of profile (ie. person, company, etc.).'
    });
    ...
    schema.addField('tags', {
      ...
      default: '[]',
      nullable: false,
      comment: 'Abritrary tags for general use.'
    });
    schema.addField('references', {
      type: 'JSON',
      default: '{}',
      nullable: true,
      comment: 'Arbitrary key/value references for general use.'
    });
    ...
    schema.addKey('profile_name_index', 'name');
    schema.addKey('profile_type_index', 'type');
    schema.addKey('profile_active_index', 'active');
    schema.addKey('profile_created_index', 'created');
    schema.addKey('profile_updated_index', 'updated');
  ...
  public delete(query: StoreSelectFilters = {}, q = '"') {
    ...
    const remove = new Delete<Profile>(this.table);
  ...
  public insert(input: Partial<Profile>) {
    ...
    const insert = new Insert<Profile>(this.table);
    return insert.values(values).returning('*');
  }

  public select<T = ProfileExtended>(query: StoreSelectQuery = {}, q = '"') {
    //extract params
  ...
  public joins(_query: StoreSelectQuery) {
    return [];
  }
  ...
  public where(builder: WhereBuilder, query: StoreSelectFilters = {}, q = '"') {
    ...
    if (query.q) {
      builder.where(
        `(${[`${q}${this.table}${q}.${q}name${q} ILIKE ?`].join(' OR ')})`,
        [`%${query.q}%`]
      );
    }
    //eq
  ...
  public update(
    ...
    const update = new Update<Profile>(this.table);
    //where
  ...
  public scalarize(values: Record<string, unknown>) {
    ...
    for (const [name, value] of Object.entries(values)) {
      ...
      if (name === 'image' && typeof value !== 'undefined') {
        const column = this.columns.image;
        scalarized.image = this.toSqlValue(
          'image',
          column.serialize(value)
        )! as ValueScalar;
        continue;
      ...
      if (name === 'name' && typeof value !== 'undefined') {
        ...
        scalarized.name = this.toSqlValue(
          'name',
          column.serialize(value)
        )! as ValueScalar;
        continue;
      ...
      if (name === 'roles' && typeof value !== 'undefined') {
        const column = this.columns.roles;
        scalarized.roles = this.toSqlValue(
          'roles',
          column.serialize(value)
        )! as ValueScalar;
        continue;
      }
      if (name === 'tags' && typeof value !== 'undefined') {
        const column = this.columns.tags;
        scalarized.tags = this.toSqlValue(
          'tags',
          column.serialize(value)
        )! as ValueScalar;
        continue;
      }
      if (name === 'references' && typeof value !== 'undefined') {
        const column = this.columns.references;
        scalarized.references = this.toSqlValue(
          'references',
          column.serialize(value)
        )! as ValueScalar;
        continue;
  ...
  public unscalarize(values: Record<string, unknown>): Partial<Profile> {
    ...
    for (const [name, value] of Object.entries(values)) {
      ...
      if (name === 'image' && typeof value !== 'undefined') {
        const column = this.columns.image;
        unscalarized.image = column.unserialize(value)! as ValuePrimitive;
        continue;
      }
      if (name === 'name' && typeof value !== 'undefined') {
        const column = this.columns.name;
        unscalarized.name = column.unserialize(value)! as ValuePrimitive;
        continue;
      }
      if (name === 'type' && typeof value !== 'undefined') {
        const column = this.columns.type;
      ...
      if (name === 'roles' && typeof value !== 'undefined') {
        const column = this.columns.roles;
        unscalarized.roles = column.unserialize(value)! as ValuePrimitive;
        continue;
      }
      if (name === 'tags' && typeof value !== 'undefined') {
        const column = this.columns.tags;
        unscalarized.tags = column.unserialize(value)! as ValuePrimitive;
        continue;
      }
      if (name === 'references' && typeof value !== 'undefined') {
        const column = this.columns.references;
        unscalarized.references = column.unserialize(value)! as ValuePrimitive;
        continue;
      }
      if (name === 'active' && typeof value !== 'undefined') {
        const column = this.columns.active;
