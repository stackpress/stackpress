...
export default function generate(directory: Directory, model: Model) {
  const filepath = model.name.toPathName('%s/events/create.ts');
  //load Profile/events/create.ts if it exists, if not create it
  ...
  //export default async function ProfileCreateEvent({ req, res, ctx }: RouteProps) {}
  const name = model.name.toPropertyName('%sCreateEvent', true);
  source.addFunction({
    isAsync: true,
    name,
    parameters: [{
