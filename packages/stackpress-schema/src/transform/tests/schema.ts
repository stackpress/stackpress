...
function getSampleValue(column: Column) {
  const value = strings.includes(column.type.name)
    ? `'foobar'`
    : column.type.enum
...
export default function generate(directory: Directory, model: Fieldset) {
  ...
  const validSample = columns.map(column => {
    const value = getSampleValue(column);
    return value === 'undefined'
      ? ''
      : `${column.name.toString()}: ${value}`;
...
`describe('<%schema%> Schema', async () => {
  ...
  it('should serialize', async () => {
    const schema = new <%classname%>();
    expect(schema.serialize(<%sample%>)).to.be.an('object');
  });
  it('should unserialize', async () => {
    const schema = new <%classname%>();
    expect(schema.unserialize(schema.serialize(<%sample%>)))
      .to.be.an('object');
  });
});`,
