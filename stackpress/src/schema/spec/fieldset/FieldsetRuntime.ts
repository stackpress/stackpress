...
export default class FieldsetRuntime {
  ...
  public assert(values: Record<string, any> = {}, strict = true) {
    ...
    for (const column of this._fieldset.columns.values()) {
      ...
      const value = column.runtime.unserialize(values[name]);
      const message = column.runtime.assert(value, strict);
  ...
  public serialize(
    ...
    for (const [ name, value ] of Object.entries(values)) {
      ...
      serialized[column.name.snakeCase] = column.runtime.serialize(
        value, 
        seed, 
        scalar
      );
  ...
  public unserialize(
    ...
    for (const [ name, value ] of Object.entries(values)) {
      ...
      unserialized[column.name.snakeCase] = column.runtime.unserialize(
        value, 
        seed, 
        scalar
      );
