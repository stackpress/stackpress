...
export default class ColumnRuntime {
   ...
   */
  public assert<V>(value: V, required = true, strict = true) {
    for (const assertion of this._column.assertion.assertions) {
      ...
      const hasNoValue = value === null 
        ...
        || value === '';
      //is.required but not required
      const isRequiredButOptional = name === 'is.required' && !required;
      //no value and not required
      const noValueAndOptional = hasNoValue && !required;
      //no value and required, but optional type or has default value
      ...
      if (typeof asserter !== 'function'
        //if not required and required
        || isRequiredButOptional
        //if not required and has no value
        || noValueAndOptional
        //if required and no value, but there is a default
      ...
      //now we can assert...
      // should we unserialize first?
      const challenge = strict 
        //dont change the value
        ? value 
        //if multiple
        : this._column.type.multiple && Array.isArray(value)
        //unserialize each value
        ? value.map((value: any) => this.unserialize(value))
        //unserialize single value
        : this.unserialize(value);
      if (!asserter(challenge, ...args)) {
    ...
    if (this._column.type.fieldset && value) {
      ...
      if (this._column.type.multiple) {
        ...
        value.forEach(
          value => errors.push(
            fieldset.runtime.assert(value, required, strict)
          )
        );
      ...
      //not a multiple
        ...
        const message = fieldset.runtime.assert(value, required, strict);
