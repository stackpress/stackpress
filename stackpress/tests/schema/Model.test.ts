...
import ColumnStore from '../../src/schema/model/ColumnStore.js';
...
describe('schema/model/Model', () => {
  ...
  it('should add column', async () => {
    ...
    expect(model.column('name')?.store).to.be.instanceOf(ColumnStore);
  }); 
});
...
describe('schema/model/ModelStore', () => {
  ...
  it('should get updated columns', async () => {
    expect(model.store.updated?.name.toString()).to.equal('name');
  });
});
